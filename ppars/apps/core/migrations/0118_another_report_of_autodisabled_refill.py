...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        #         note = log.note
        #         refill_id = re.search('System updated AutoRefill: (.*)\\nENABLED: from True to False; ', note)
        #         autorefill_id = refill_id.group(1).strip()
        #         if refill_id and autorefill_id.isdigit() and\
        #                 orm.AutoRefill.objects.filter(id=long(autorefill_id), enabled=False).exists():
        #             auto_refill = orm.AutoRefill.objects.get(id=long(autorefill_id))
        #             auto_refill.enabled = True
        #             auto_refill.save()
        #             message += '%s,' % autorefill_id
        #     message = message[:-1]
        #     message += '\n'
