...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # That supposed to be script but it was created as migration

        # message = 'Report after fix for scheduled refills that were disabled by system automatically:\nCompanies:\n'
        # for company in orm.CompanyProfile.objects.all():
        #     message += '\nfor %s (%s) scheduled refills that were enabled:\n ' % (company, str(company.id))
        #     for log in orm.Log.objects.filter(company=company, note__startswith='System updated AutoRefill: ',
        #                                       note__endswith='ENABLED: from True to False; '):
        #         note = log.note
        #         autorefill_id = re.search('System updated AutoRefill: (.*)\nENABLED: from True to False; ', note)
        #         if autorefill_id and autorefill_id.group(1).isdigit() and\
        #                 orm.AutoRefill.objects.filter(id=long(autorefill_id.group(1)), enabled=False).exists():
        #             auto_refill = orm.AutoRefill.objects.get(id=long(autorefill_id.group(1)))
        #             auto_refill.enabled = True
        #             auto_refill.save()
        #             message += '%s,' % autorefill_id.group(1)
        #     message = message[:-1]
        #     message += '\n'
        #
        # orm.Log.objects.create(
        #     company=orm.CompanyProfile.objects.get(superuser_profile=True),
        #     note=message)
