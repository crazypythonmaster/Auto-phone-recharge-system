...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for ccc in orm.CreditCardCharge.objects.all():
            ccc.customer = ccc.autorefill.customer
            amount = 0.0
            if ccc.used:
                amount = decimal.Decimal(ccc.amount)
                orm.TransactionCharge.objects.create(charge=ccc, transaction=ccc.transaction, amount=amount)
            ccc.sum = amount
            ccc.save()
