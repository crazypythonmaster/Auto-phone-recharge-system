...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for customer in orm.Customer.objects.all():
            sms_email_numbers = customer.sms_email.split(',')
            for phone in orm.PhoneNumber.objects.filter(reduce(operator.or_,
                                                               (Q(number=val)
                                                                for val in sms_email_numbers)),
                                                        customer=customer):
                phone.use_for_sms_email = True
                phone.sms_gateway_id = customer.sms_gateway.id
                phone.save()
