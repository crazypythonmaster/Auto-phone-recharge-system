...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for plan in orm.Plan.objects.all():
            plan.cost = decimal.Decimal(plan.plan_cost.replace('$', ''))
            plan.save()
        for ccc in orm.CreditCardCharge.objects.all():
            ccc.amount1 = decimal.Decimal(ccc.amount)
            ccc.save()
        for transaction in orm.Transaction.objects.all():
            if transaction.cost:
                transaction.cost1 = decimal.Decimal(transaction.cost)
            else:
                transaction.cost1 =0.0
            if transaction.profit:
                transaction.profit1 = decimal.Decimal(transaction.profit)
            else:
                transaction.profit1 =0.0
            transaction.save()
        for pd in orm.PlanDiscount.objects.all():
            pd.discount1 = decimal.Decimal(pd.discount)
            pd.save()
