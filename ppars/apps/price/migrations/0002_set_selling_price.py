...
class Migration(DataMigration):
    ...
    depends_on = (
        ("core", "0002_auto__add_field_customer_selling_price_level"),
    )
    ...
    def forwards(self, orm):
        ...
        while i < 4:
            i += 1
            orm.SellingPriceLevel.objects.create(level=i)
        for company in orm['core.CompanyProfile'].objects.filter(superuser_profile=False):
            for plan in orm['core.Plan'].objects.all():
                for price_level in orm.SellingPriceLevel.objects.all():
                    if '2' == price_level.level:
                        selling_price = plan.second_selling_price
                    elif '3' == price_level.level:
                        selling_price = plan.third_selling_price
                    else:
                        selling_price = plan.first_selling_price
                    orm.PlanSellingPrice.objects.create(carrier=plan.carrier,
                                                        plan=plan,
                                                        company=company,
                                                        price_level=price_level,
                                                        selling_price=selling_price)

        for customer in orm['core.Customer'].objects.all():
            customer.selling_price_level = orm.SellingPriceLevel.objects.get(level=customer.price_level)
            customer.save()
