...
class Migration(DataMigration):
    ...
    depends_on = (
        ("core", "0020_auto__add_field_creditcardcharge_refund_id__add_field_creditcardcharge"),
    )
    ...
    def forwards(self, orm):
        ...
        for cccharge in orm['core.CreditCardCharge'].objects.all():
            orm.Charge.objects.create(
                id=cccharge.id,
                company=cccharge.company,
                autorefill=cccharge.autorefill,
                customer=cccharge.customer,
                creditcard=cccharge.creditcard,
                used=cccharge.used,
                amount=cccharge.amount,
                tax=cccharge.tax,
                summ=cccharge.summ,
                atransaction=cccharge.atransaction,
                payment_getaway=cccharge.payment_getaway,
                status=cccharge.status,
                refund_id=cccharge.refund_id,
                refund_status=cccharge.refund_status,
                refunded=cccharge.refunded,
                adv_status=cccharge.adv_status,
                created=cccharge.created,
            )
        for transactioncharge in orm['core.TransactionCharge'].objects.all():
            orm.TransactionCharge.objects.create(
                charge_id=transactioncharge.charge.id,
                transaction=transactioncharge.transaction,
                amount=transactioncharge.amount,
                created=transactioncharge.created
            )
